...
data {
  ...
  int<lower = 1> N_subj;
  int<lower = 1> N_tot;
  int<lower = 1> subj [N_tot];
  int<lower = 1> N_rt;
  
  //stimuli : go vs nogo (i.e 1 vs 0)
  vector[N_tot] GO;
  vector[N_tot] NOGO;
  int<lower = 0, upper = 1> SE [N_tot];
  vector[N_tot] TN;
...
model {
  ...
  target += normal_lpdf(beta_om_se | 0, 1);
  target += normal_lpdf(lambda | 0, 1);

  // LN regression model
  target += normal_lpdf(sigma | .1, .2)
    - normal_lccdf(0 | .1, .2);
  
  // mu
  target += normal_lpdf(intercept | -.9, 1);
  target += normal_lpdf(beta | 0, 1);
  target += normal_lpdf(beta_nogo | 0, 1);
  target += normal_lpdf(beta_se | 0, 1);
  target += normal_lpdf(beta_p_se | 0, 1);
  target += normal_lpdf(beta_tn | 0, 1);
  target += normal_lpdf(beta_tn_se | 0, 1);
  
  target += normal_lpdf(tau_u[1:N_re] | .1, .1)
  ...
  for(n in 1:N_tot) {
        ...
        v   = v + Phi(lambda + u[subj[n],3]) * delta_v; // volatility update 
    ...
    // lognormal
    log_lik[i] = lognormal_lpdf(rt[n] | mu, sigma); 
    i = i + 1;
    }
    // Initialize if subj !=
    nn = n < (N_tot) ? n : n-1;
  
    v = subj[n] != subj[nn+1] ? exp(omega + u[subj[nn+1], 1] + SE[nn] * (beta_om_se + u[subj[nn+1], 2])) : v; // Initialize volatilities
    w = subj[n] != subj[nn+1] ? exp(omega + u[subj[nn+1], 1] + SE[nn] * (beta_om_se + u[subj[nn+1], 2])) : w;  // Initialize posterior variances
    m = subj[n] != subj[nn+1] ? 0 : m; // Initialize predictions
